// Vishnu Purana
// One of the eighteen Mahapuranas, focusing on Lord Vishnu

export const vishnuPuranaObject = {
  id: 4,
  title: 'Vishnu Purana',
  type: 'purana',
  summary: 'One of the eighteen Mahapuranas, this sacred text glorifies Lord Vishnu and describes the creation, maintenance, and dissolution of the universe.',
  imageUrl: '/assets/images/vishnu_purana.png', // To be added
  introduction: "The Vishnu Purana is one of the eighteen Mahapuranas and is considered one of the most important Puranas. It describes the creation of the universe, the genealogy of gods and sages, the avatars of Vishnu, and provides guidelines for living a righteous life. It consists of six books (amshas) with approximately 23,000 verses.",
  hasAudio: true,
  hasIllustrations: true,
  books: [
    {
      number: 1,
      title: "Book One: Creation",
      summary: "The creation of the universe, the genealogy of gods, sages, and the first Manu. Describes the cosmic cycles and the nature of time.",
      chapters: 22,
      keyTopics: [
        "Creation of the universe by Brahma",
        "The story of Dhruva's devotion",
        "Genealogy of gods and sages",
        "The story of Prahlada and Hiranyakashipu",
        "Cosmic time cycles (yugas)"
      ],
      audioUrl: null,
      illustration: null,
      stories: [
        {
          title: "Dhruva's Devotion",
          summary: "The story of young prince Dhruva who through intense devotion to Vishnu attained the Pole Star position.",
          hasIllustration: true,
          illustrationUrl: null
        },
        {
          title: "Prahlada and Hiranyakashipu",
          summary: "The demon king Hiranyakashipu's conflict with his devotee son Prahlada, leading to Vishnu's Narasimha avatar.",
          hasIllustration: true,
          illustrationUrl: null
        }
      ]
    },
    {
      number: 2,
      title: "Book Two: The Earth and Its Divisions",
      summary: "Describes the geography of Earth, its divisions, mountains, rivers, and the organization of society.",
      chapters: 16,
      keyTopics: [
        "Geography of Jambudvipa (ancient India)",
        "The seven continents and seven oceans",
        "Sacred rivers and mountains",
        "The organization of varnas (social classes)",
        "Duties of different life stages (ashramas)"
      ],
      audioUrl: null,
      illustration: null
    },
    {
      number: 3,
      title: "Book Three: Divisions of Time and Social Duties",
      summary: "Explains the divisions of time, the duties of the four varnas and four ashramas, and rules for daily conduct.",
      chapters: 18,
      keyTopics: [
        "Measurement of time from atoms to yugas",
        "Duties of Brahmanas, Kshatriyas, Vaishyas, and Shudras",
        "The four ashramas: student, householder, hermit, renunciate",
        "Rules for daily worship and rituals",
        "The path to liberation"
      ],
      audioUrl: null,
      illustration: null
    },
    {
      number: 4,
      title: "Book Four: Royal Dynasties",
      summary: "Genealogies of kings from the Solar and Lunar dynasties, including the story of Krishna and the Pandavas.",
      chapters: 24,
      keyTopics: [
        "Solar dynasty (Suryavansha) - Rama's lineage",
        "Lunar dynasty (Chandravansha) - Krishna's lineage",
        "The story of Krishna's birth and childhood",
        "Krishna's role in the Mahabharata",
        "Future kings and the Kali Yuga"
      ],
      audioUrl: null,
      illustration: null,
      stories: [
        {
          title: "Krishna's Birth and Childhood",
          summary: "The miraculous birth of Krishna in prison, his childhood in Gokul, and his divine pastimes.",
          hasIllustration: true,
          illustrationUrl: null
        }
      ]
    },
    {
      number: 5,
      title: "Book Five: Krishna's Life",
      summary: "Detailed account of Krishna's life from his youth in Vrindavan to his role in establishing dharma.",
      chapters: 38,
      keyTopics: [
        "Krishna's pastimes in Vrindavan",
        "Krishna and the Gopis",
        "Killing of various demons",
        "Krishna in Mathura and Dwaraka",
        "Krishna's teachings and divine play"
      ],
      audioUrl: null,
      illustration: null,
      stories: [
        {
          title: "Krishna and the Gopis",
          summary: "The divine love between Krishna and the Gopis, exemplifying the highest devotion.",
          hasIllustration: true,
          illustrationUrl: null
        },
        {
          title: "Lifting Govardhan Hill",
          summary: "Krishna lifts the Govardhan mountain to protect the villagers from Indra's wrath.",
          hasIllustration: true,
          illustrationUrl: null
        }
      ]
    },
    {
      number: 6,
      title: "Book Six: Dissolution and Liberation",
      summary: "Describes the end of Krishna's earthly avatar, the Kali Yuga, and the path to moksha (liberation).",
      chapters: 8,
      keyTopics: [
        "Krishna's departure from Earth",
        "The beginning of Kali Yuga",
        "Signs of Kali Yuga",
        "The path to liberation through devotion",
        "The ultimate dissolution of the universe"
      ],
      audioUrl: null,
      illustration: null
    }
  ]
};
