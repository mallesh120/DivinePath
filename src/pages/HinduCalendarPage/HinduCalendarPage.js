...
const HinduCalendarPage = () => {
  ...
  // Get location and festivals
  const userLocation = getLocationForFestivals();
  const allFestivals = useMemo(() => 
    getAllFestivalsWithCalculated(userLocation, minYear, maxYear),
    [userLocation]
  );
  ...
  return (
    <div className="hindu-calendar-page">
      <div className="calendar-page-header">
        <h1 className="calendar-page-title">🗓️ Hindu Calendar & Panchangam</h1>
        <p className="calendar-page-subtitle">
          Complete Hindu calendar with daily Panchangam, Tithi, Nakshatra, Yoga, festivals, and auspicious timings
        </p>
      </div>

      {/* Today's Panchangam Section */}
      {panchangamLoading && showTodayPanchangam && (
        <div className="panchangam-loading">Loading today's Panchangam...</div>
      )}
      
      {showTodayPanchangam && panchangamData && !panchangamLoading && (
        <div className="today-panchangam-section">
          <div className="today-panchangam-header">
            <h2 className="today-panchangam-title">📅 Today's Panchangam</h2>
            <button 
              className="toggle-panchangam-btn"
              onClick={() => setShowTodayPanchangam(false)}
              aria-label="Hide today's panchangam"
            >
              ✕
            </button>
          </div>

          <div className="today-panchangam-meta">
            <span className="panchangam-meta-item">📍 {panchangamData.meta.location}</span>
            <span className="panchangam-meta-item">📅 {panchangamData.meta.day}</span>
          </div>

          {panchangamError && <div className="panchangam-error">{panchangamError}</div>}

          <div className="today-panchangam-content">
            {/* Almanac Section */}
            <div className="panchangam-section">
              <h3 className="panchangam-section-title">📖 Almanac</h3>
              <div className="panchangam-grid">
                {Object.entries(panchangamData.almanac).map(([key, item]) => {
                  const displayValue = typeof item === 'object' && item !== null ? item.name : item;
                  const subText = typeof item === 'object' && item !== null ? item.endTime : null;
                  return (
                    <div key={key} className="panchangam-row">
                      <div className="panchangam-label">{key}</div>
            ...
            {/* Solar & Lunar Section */}
            <div className="panchangam-section">
              <h3 className="panchangam-section-title">☀️ Solar & Lunar</h3>
              <div className="panchangam-grid">
                {Object.entries(panchangamData.solarLunar).map(([key, value]) => (
                  <div key={key} className="panchangam-row">
                    <div className="panchangam-label">{key}</div>
                    <div className="panchangam-value">{value}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="panchangam-columns">
              <div className="panchangam-section auspicious">
                <h3 className="panchangam-section-title">✨ Auspicious Timings</h3>
                <div className="panchangam-grid">
                  {Object.entries(panchangamData.auspicious).map(([key, value]) => (
                    <div key={key} className="panchangam-row">
                      <div className="panchangam-label">{key}</div>
                      <div className="panchangam-value">{value}</div>
                    </div>
              ...
              <div className="panchangam-section inauspicious">
                <h3 className="panchangam-section-title">⚠️ Inauspicious Timings</h3>
                <div className="panchangam-grid">
                  {Object.entries(panchangamData.inauspicious).map(([key, value]) => (
                    <div key={key} className="panchangam-row">
                      <div className="panchangam-label">{key}</div>
                      <div className="panchangam-value">{value}</div>
                    </div>
      ...
      {/* Show Panchangam button when hidden */}
      {!showTodayPanchangam && (
        <button 
          className="show-panchangam-btn"
          onClick={() => setShowTodayPanchangam(true)}
        >
          📅 Show Today's Panchangam
        </button>
      )}
          ...
          <div className="legend-section">
            ...
            <div className="legend-items">
              ...
              <div className="legend-item">
                ...
                <span className="legend-text">Day with festivals (click to view)</span>
              </div>
              <div className="legend-item">
            ...
            <div className="panchang-details-section">
              ...
              <div className="calendar-festival-list">
                {selectedDate.festivals.map((festival, idx) => (
                  <div 
                    key={idx} 
                    className="calendar-festival-item clickable"
...
export default HinduCalendarPage;
